...
static callback_list_elem_p __callback_get(callback_list_p const cbl, unsigned int index)
{
	return(&cbl->cble[index]);
}

static void __callback_set(callback_list_p const cbl,
	callback_fn const fn, void* param, unsigned int index)
{
	callback_list_elem_p const cble = __callback_get(cbl, index);

	cble->fn = fn;
	cble->param = param;
}

static void __callback_move(callback_list_p const cbl,
...
static void _callback_list_init(callback_list_p const cbl,
...
static void _register_callback_fifo(callback_list_p const cbl,
...
static void _register_callback_lifo(callback_list_p const cbl,
	callback_fn const fn, void *const param)
{
	if(cbl->count >= (cbl->limit - 1))
		LOG_ACTION(abort());
...
void callback_list_init(callback_list_p const cbl,
	unsigned int additional, unsigned int type)
{
	assert(0 == additional);
...
void callback_list_alloc_init(callback_list_h const h2cbl,
	unsigned int additional, unsigned int type)
{
...
void callback_list_process(callback_list_p const cbl)
{
	DEBUG(LOG_START(">> cbl = 0x%08" PRIxPTR, (uintptr_t)cbl));
	DEBUG(_LOG_(", count = 0x%08x", cbl->count));
	DEBUG(LOG_END(", limit = 0x%08x", cbl->limit));
...
void callback_list_register_callback(callback_list_p const cbl,
	callback_fn const fn, void *const param)
{
	assert(0 != cbl);
	assert(0 != cbl->type);
